...
class ImageVerification {
    ...
    function randomID() {
        ...
        $this->DB->insert( 'INSERT INTO botcode SET ' . $this->DB->compose( $SQL ), __FILE__, __LINE__ );
    ...
    function verified( $token, $code ) {
        $sql = $this->DB->select( 'SELECT botID FROM botcode
                                   WHERE token = "' . addslashes( $token ) . '" AND
                                         code  = "' . (int)$code . '"', __FILE__, __LINE__ );

        if( $this->DB->numrows( $sql ) > 0 ) {
            $row = $this->DB->fetch( $sql );

            // Has been verified, we don't need it anymore.
            $this->DB->delete( 'DELETE FROM botcode WHERE botID = "' . (int)$row['botID'] . '"', __FILE__, __LINE__ );
