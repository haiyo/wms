 ...
 /**
  ...
  protected function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
	  ...
	  if ($this->SMTPDebug) {
        $this->outPutError( $this->Lang('Invalid recipient array').': '.$kind );
      }
      return false;
    }
    ...
    if (!self::ValidateAddress($address)) {
	  ...
	  if ($this->SMTPDebug) {
        $this->outPutError( $this->Lang('invalid_address').': '.$address );
      }
      return false;
    }
    if ($kind != 'Reply-To') {
 ...
 */
  ...
  protected function PostSend() {
    try {
	  ...
	  if ($this->SMTPDebug) {
        $this->outPutError( $e->getMessage().'<br />' );
      }
      return false;
    }
  }
  ...
  */
  public function outPutError( $errMsg ) {
      ...
      //echo 'Connecting Host: ' . $this->Host . ':' . $this->Port . ' (' . $this->SMTPSecure . ') ' . str_replace( array("\r\n", "\n", "\r"), '', $MXString->makeLink( $errMsg ) ) . '<br />';
      */
      echo str_pad('',4096)."\n";
      ob_flush();
      flush();
