...
class SecureImage {
   ...
   * @var string
   */
  var $text_color = "#cc3838,#1523e4,#1ec808,#f67710,#f310f6";
   ...
   * @var int
   */
  var $text_transparency_percentage = 70;
  ...
  function arcLines()
  {
    ...
    imagearc($this->im, $this->image_width * .75, $ypos, $width+60, $height, $start, $end, $linecolor);
  }
  ...
  function drawWord()
  {
    ...
    else
    {
        ...
        if( $this->use_transparent_text === true )
        {
            ...
            $font_color = imagecolorallocatealpha($this->im, hexdec(substr($color, 1, 2)), hexdec(substr($color, 3, 2)), hexdec(substr($color, 5, 2)), $alpha);
        }
        else
        {
            //no transparency
            $font_color = imagecolorallocate($this->im, hexdec(substr($color, 1, 2)), hexdec(substr($color, 3, 2)), hexdec(substr($color, 5, 2)));
        }
