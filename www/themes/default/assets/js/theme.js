    ...
    $window.on('scroll', function(){
        if( $window.width() < 992 ) {
            ...
            if( windowOffset > tabNavOffset ) {
                $tabNav.removeClass('in');
            }
        }
    });

    $toggleMenuButton.on('click', function(){
        if( !$tabNav.hasClass('in') ) {
...
(function($) {
    ...
    var LockScreen = {
        ...
        getUserInfo: function() {
            ...
            $info    = $( '#userbox' );
            picture  = $info.find( '.profile-picture images' ).attr( 'data-lock-picture' );
        ...
        buildTemplate: function( userinfo ) {
            return [
                ...
                '<images id="LockUserPicture" src="{{picture}}" alt="John Doe" class="images-circle user-image" />',
